...
입력은 [문서관리카드 정보](시스템 메타데이터)와 [공문 본문], [첨부파일: 파일명] 섹션으로 나뉘어 올 수 있습니다.
[공문 본문]이 있으면 요약의 중심으로 삼고, 카드 정보는 공개여부·첨부파일 등 보조 정보로만 쓰세요.
[첨부파일] 섹션이 있으면 그 내용도 요약에 반영하세요. 특히 본문이 "붙임 참조"로 넘긴
세부사항(제출 기한·양식·제출처·대상)은 첨부에서 찾아 deadline·actions·key_points에 채우세요.
첨부에서 나온 정보는 어느 첨부인지 알 수 있게 쓰세요 (예: "첨부 서식의 제출기한 8.10.").
...

{
  ...
  "typos": [{"before": "원문에 실제로 있는 틀린 표기 그대로", "after": "고친 표기", "reason": "맞춤법·띄어쓰기·오타·조사 호응·중복 중 하나로 짧게"}]
}
...
- 입력에 [첨부파일: 파일명] 섹션이 있으면 본문과 첨부 간 금액·일정·대상의 불일치를 확인해
  improvements/checks에 반영하세요.

오탈자(typos) 원칙:
- before는 원문에 실제로 존재하는 문자열을 한 글자도 바꾸지 말고 그대로 옮기세요.
  원문에 없는 문자열을 만들면 안 됩니다. 필요하면 틀린 부분을 포함한 어절 단위로 자르세요.
- 명백한 오류만 잡으세요: 오타, 맞춤법, 띄어쓰기, 조사 호응(을/를·이/가), 같은 단어 중복.
  단순 문체 취향, 개조식 표현, 기관에서 관행적으로 쓰는 표기는 오탈자가 아닙니다.
- 검수 범위는 [공문 본문](없으면 [문서관리카드 정보])만. [첨부파일] 내용은 제외하세요.
- 확신이 없으면 넣지 마세요. 최대 8개, 없으면 빈 배열 [].
- 오탈자는 typos에만 담고 improvements에 반복하지 마세요. 오탈자만으로 status를 낮추지 마세요.
  단, 금액·날짜·기관명의 오기처럼 결재 판단에 영향을 주는 오류는 improvements/checks에도 쓰세요.
